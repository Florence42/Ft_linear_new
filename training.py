...
'''' print the graph'''
def print_graph(tab, beta1, beta0, beta1_f, beta0_f, J):
    import matplotlib
    ...
    b.append(tmp)
    plt.plot(c, b, "g--", label= "least-squares estimation")
    ...
    plt.ylabel('Cost/10.000')
    plt.plot(J)

    plt.show()

''' classical linear regression (least-squares estimation)'''
...
def print_cost(tab, beta1, beta0):
    m = len(tab)
    print("\n Mileage -- Price -- Pred. --   Gap (Real Price - Predict)")
    for i in range (m):
...
''' gradient descent '''
def make_learning(list_price, list_km, beta1, beta0, iter = 0, J_history = []):
    deriv_beta0 = 0.
    deriv_beta1 = 0.
    #ilearn = 0.1
    ilearn = 0.9
    ...
    while (True):
        ...
        iter +=1
        J_history.append( make_cost(list_price, list_km, beta1, beta0 * scale )/scale)
        
        if abs(deriv_beta0) < float(0.000000001) and abs(deriv_beta1) < float(0.000000001):  
            #cost = make_cost(list_price, list_km, beta1, beta0 * scale )/10000
            #print("--------------FLO :", cost)     
            return (beta1, beta0 * scale, iter, J_history)
        #print("beta0", beta0)      
        #print("beta1", beta1)

        beta0 = beta0 - deriv_beta0
        beta1 = beta1 - deriv_beta1
        #cost = make_cost(list_price, list_km, beta1, beta0 * scale)
        #print("cost ", cost)
...
def treat_csv_file(inputfile, outputfile):
    with open(inputfile, newline='') as csvfile:
        ...
        print("\n")
        print("-----------------------------------------------------------------------\n")
        print("The model takes on the standard form : Price = beta0 + (beta1 * Mileage)") 
        print("\n")
        print("cost = Sum[(estimated-real)^^2]/(2*m)   m = number of values in the data set") 

        """ classical linear regression (least-squares estimation)"""
        beta1_f, beta0_f = make_regression(list_price, list_km)
        print("\n Linear regression results (least-squares estimation) :")
        ...
        beta1, beta0, iter, J_history = make_learning(list_price, list_km, beta1=0., beta0= 0., iter = 0, J_history=[])
        ...
        print_graph(tab, beta1, beta0, beta1_f, beta0_f, J_history)
